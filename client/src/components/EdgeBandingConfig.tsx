...
import type { EdgeBand, EdgeBandSides } from '@/lib/optimizer';
...
export function EdgeBandingConfig({ onApply, onCancel }: EdgeBandingConfigProps) {
  ...
  const handleApply = () => {
    ...
    const edgeBand: EdgeBand = {
      ...
      sides: {
        top: sides.top ? parseFloat(thickness) : 0,
        bottom: sides.bottom ? parseFloat(thickness) : 0,
        left: sides.left ? parseFloat(thickness) : 0,
        right: sides.right ? parseFloat(thickness) : 0,
      },
    };
